...
public class MemberDao {
	...
	public int loginCheck(String mId, String mPw) {
		...
		PreparedStatement pstmt = null;
		ResultSet         rs    = null;
		String sql = "SELECT * FROM MEMBER WHERE MID=? AND MPW=? AND MSTATE = 0";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, mId);
	...
	public MemberDto getMember(String mId) {
		...
		try {
			...
			if(rs.next()) {
				...
				int    mState   = rs.getInt("mState");
				member = new MemberDto(mId, mPw, mName, mAddress, mTel, mEmail, mBirth, mRdate, mState);
			}
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}finally {
	...
	public ArrayList<MemberDto> allMember(int startRow, int endRow){
		...
		try {
			...
			while(rs.next()) {
				...
				Date   mRdate = rs.getDate("mRdate");
				int    mState   = rs.getInt("mState");
				members.add(new MemberDto(mId, mPw, mName, mAddress, mTel, mEmail, mBirth, mRdate, mState));
			}
		} catch (Exception e) {
	...
	public int getMemberTotCnt() {
		...
		PreparedStatement pstmt = null;
		ResultSet         rs    = null;
		String sql = "SELECT COUNT(*) FROM MEMBER WHERE MSTATE=0";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	//회원목록(startRow ~ endRow) 
		public ArrayList<MemberDto> allMemberWithdrawal (int startRow, int endRow){
			ArrayList<MemberDto> members = new ArrayList<MemberDto>();
			Connection        conn  = null;
			PreparedStatement pstmt = null;
			ResultSet         rs    = null;
		...
		//가입한 회원 전체 명수
		public int memberWithdrawalTotCnt() {
			int totCnt = 0;
			Connection        conn  = null;
			PreparedStatement pstmt = null;
			ResultSet         rs    = null;
			String sql = "SELECT COUNT(*) FROM MEMBER WHERE MSTATE=1";
			try {
				conn = getConnection();
				pstmt = conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
	...
	public int delteMember(String mId) {
		...
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE MEMBER SET MSTATE = 1 WHERE MID=?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, mId);
