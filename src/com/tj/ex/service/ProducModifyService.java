...
public class ProducModifyService implements Service {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			int result = pDao.productUpdate(pName, pType, pFilename, pOrign, pWeight, pPart, pUse, pPrice, pStock, pCode);
			System.out.println();
			if(result == ProductDao.SUCCESS) {
				
				request.setAttribute("productInsert", "상품수정 성공");
			}else {
				request.setAttribute("erroMsg", "상품수정 실패");
			}
